...
public class GLUtils {
	 ...
	 * -z direction. <br>
	 * <p>
	 * Restrictions on nearDistance and farDistance are as follows:
	 * <li>  <code>nearDistance</code> and <code>farDistance</code> must be <code>>= 0</code>.
	 * <li> <code>farDistance</code> must be greater than <code>nearDistance</code>.
	 * 
	 * @param aspectRatio
	 ...
	 * 
	 */
	public static Matrix4f createProjectionMatrixFov(float aspectRatio, float fieldOfView,
			float nearDistance, float farDistance){
		validateNearFarDistances(nearDistance, farDistance);
		...
		projectionMatrix.m22 = -((farDistance + nearDistance) / frustum_length);
		projectionMatrix.m23 = -1;
		projectionMatrix.m32 = -((2 * nearDistance * farDistance) / frustum_length);
	 ...
	 * @param nearDistance - z distance to the near frustum face.
	 * @param farDistance - z distance to the far frustum face.
	 * 
	 * @return A Matrix4f representing the perspective projection matrix.
	 */
	public static Matrix4f createProjectionMatrix(float left, float right, float bottom,
			float top, float nearDistance, float farDistance) {
		validateNearFarDistances(nearDistance, farDistance);
		...
		float frustum_length = farDistance - nearDistance;   // z range.
		
		projectionMatrix.m00 = 2 * nearDistance / width;
		projectionMatrix.m02 = (right + left) / width;
		projectionMatrix.m11 = 2 * nearDistance / height;
		projectionMatrix.m12 = (top + bottom) / height;
		projectionMatrix.m22 = -1 * (farDistance + nearDistance) / frustum_length;
		projectionMatrix.m23 = -2 * farDistance * nearDistance / frustum_length;
	...
	public static Matrix4f createProjectionMatrix(float width, float height,
		...
		return createProjectionMatrix(left, right, bottom, top, nearDistance,
				farDistance);
	}
	 ...
	 * @param nearDistance - z distance to the near face.
	 * @param farDistance - z distance to the far face.
	 * 
	...
	public static Matrix4f createOrthoProjectionMatrix(float left, float right,
			float bottom, float top, float nearDistance, float farDistance) {
		validateNearFarDistances(nearDistance, farDistance);
		...
		projectionMatrix.m22 = -2 / (farDistance - nearDistance);
		
		// Set translation components.
		projectionMatrix.m03 = -1 * (right + left) / (right - left);
		projectionMatrix.m13 = -1 * (top + bottom) / (top - bottom);
		projectionMatrix.m23 = -1 * (farDistance + nearDistance) / (farDistance - nearDistance);
	...
	private static void validateNearFarDistances(float nearDistance, float farDistance){
		if (nearDistance < 0){
			throw new IllegalArgumentException("Error: nearDistance cannot be negative");
		}
		if (farDistance < 0){
			throw new IllegalArgumentException("Error: farDistance cannot be negative");
		}
		if (nearDistance == farDistance){
			throw new IllegalArgumentException("Error: nearDistance cannot equal farDistance!");
		}
		if (nearDistance > farDistance){
			throw new IllegalArgumentException("Error: nearDistance cannot be greater than farDistance!");
		}
	}
